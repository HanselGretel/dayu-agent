...
- API Key、Token、密码、私钥、会话 Cookie 等私人凭证，禁止硬编码进本仓库任何会被版本管理或对外分发的内容，包括但不限于：源码、配置 JSON、测试、文档、脚本、示例输出、开发回顾。
- 凭证只能通过环境变量读取；配置里只允许写环境变量名（如 `api_key_env: CURSOR_API_KEY`），禁止写真实密钥值。
- 文档与注释示例只能使用占位符（如 `cursor_...`、`sk-xxxxxxxx`）；测试只能使用明显假值（如 `cursor_test`、`sk-test`），不得使用真实或疑似真实的密钥。
- 禁止将 `.env`、含真实密钥的本地 workspace 配置、trace/日志/导出文件中的敏感字段提交或上传到仓库。
- 任何代码、配置、文档修改在提交前，必须确认未引入上述敏感信息；若已泄露，立即轮换密钥并清除误提交内容，不得保留在仓库历史中。
